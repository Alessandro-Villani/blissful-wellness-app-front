...
<template>
    <div class="massage-card mb-5 row align-items-center" :class="index % 2 === 0 ? 'flex-row-reverse' : 'flex-row'">
        <div class="col-5 d-flex flex-column align-items-center justify-content-between mb-2">
            <h4>{{ massage.name }}</h4>
            <img class="massage-pic border rounded-circle img-fluid"
        ...
        <div :class="index % 2 === 0 ? 'text-end' : 'text-start'" class="col-7 mb-2">
            ...
            <p class="price mb-0"><b>Price/h: </b>₱{{ massage.pricePerHour }}</p>
        </div>
        <div class="col-12 text-center">
            <button class="btn btn-primary" @click="$emit('book', massage)">Book</button>
        </div>
    </div>
</template>
